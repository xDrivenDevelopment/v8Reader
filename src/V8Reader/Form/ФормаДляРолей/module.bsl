...
Процедура ПеребратьРекурсивно(НоваяСтрока, Строка, ВерхняяСтрокаНужна, Родитель, ИспользоватьРЛС, ВерхнееРЛСНужно, СтрокаДополнения);
	...
	Для Каждого стр Из Строка.Строки Цикл
		...
		Если ТаблицаПрав.Найти(Стр.Идентификатор, "Идентификатор") <> Неопределено Тогда
			...
			//Если данных по строке нет и подчиненные строки для нее тоже в таблице прав отсутствуют, тогда в дереве объектов метаданных ее можно удалить
		ИначеЕсли СтрокаНужна = Ложь Тогда
...
Процедура ДостроитьТаблицуДаннымиВторойРоли(МассивОбщихСвойствРоли)
	...
	//в уже существующую таблицу РЛС добавляем колонки, соответствующие значениям РЛС для сравниваемого объекта и полей РЛС для него же
	...
	Если МассивРеквизитовОбъекта.Количество() > 0 Тогда
		Для Каждого Вр Из МассивРеквизитовОбъекта  Цикл
			Если СтрокаПрава <> Неопределено И Узел.ДочерниеУзлы[2].ТекстовоеСодержимое <> "0" И СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных] <> Неопределено Тогда
				Ключ = СтрЗаменить(Узел.ДочерниеУзлы[3].ДочерниеУзлы[0].ТекстовоеСодержимое, "-", "v");
				Если СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных].Свойство(Ключ) Тогда
					Если Вр.ОбъектСРеквизитами = СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных][Ключ] Тогда
						НоваяСтрока = Вр;
						Прервать;
					КонецЕсли;
				КонецЕсли;
...
Процедура ДополнитьОбъектСРеквизитами(УзелДляАнализа, СтрокаПрава, НоваяСтрока, МассивРеквизитовОбъекта)
	...
	Если НоваяСтрока = Неопределено  Тогда
		...
		Если УзелДляАнализа.ТекстовоеСодержимое <> "0" Тогда
			Если СтрокаПрава <> Неопределено Тогда
				Если СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных] <> Неопределено Тогда
					Ключ = СтрЗаменить(УзелДляАнализа.СледующийСоседний.ДочерниеУзлы[0].ТекстовоеСодержимое, "-", "v");
					Если СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных].Свойство(Ключ) Тогда
						НоваяСтрока.ОбъектСРеквизитами = СоответствиеСтандартныхРеквизитов[СтрокаПрава.ВидМетаданных][Ключ];
					Иначе
						НоваяСтрока.ОбъектСРеквизитами = "СтандартныйРеквизит" + УзелДляАнализа.СледующийСоседний.ДочерниеУзлы[0].ТекстовоеСодержимое;
					КонецЕсли
				Иначе
...
Процедура СтатистическиеДанныеСравнения(МассивНеопределенныхСтрок)
	...
	НеизмененныеСтрокиПрав = ПолнаяТаблицаПрав.НайтиСтроки(Новый Структура("ЕстьИзменения", 1));
	СчетчикОтличийПрав = ПолнаяТаблицаПрав.Количество() - НеизмененныеСтрокиПрав.Количество();
	НеизмененныеСтрокиШаблонов = ТаблицаШаблонов.НайтиСтроки(Новый Структура("ЕстьИзменения", Ложь));
	СчетчикОтличийШаблонов = ТаблицаШаблонов.Количество() - НеизмененныеСтрокиШаблонов.Количество();
	НеизмененныеСтрокиРЛС = ПолнаяТаблицаРЛС.НайтиСтроки(Новый Структура("ЕстьИзмененияРЛС", 1));
	СчетчикОтличийРЛС = ПолнаяТаблицаРЛС.Количество() - НеизмененныеСтрокиРЛС.Количество();
