...
Процедура ГиперссылкаНажатие(Элемент)
	...
	ТекстФормы = ДекомпиляторФормы(Истина);
...
Процедура Гиперссылка1Нажатие(Элемент)
	...
	ТекстФормы = ДекомпиляторФормы(Истина, 2);
	ТекстФормы = ТекстСвойствФормы + Символы.ПС + ТекстФормы;
	ВиртФорма = ПолучитьФорму("ВиртуальнаяФорма");
	ВиртФорма.ТолькоПросмотр = Истина;
	ВиртФорма.ТекстФормы = ТекстФормы;
...
Процедура ГиперссылкаФормыНажатие(Элемент)
	Если СравнениеФорм = Истина Тогда
		СравнениеФайлов = ТекстовоеСравнениеВоВременныхФайлах(ДекомпиляторФормы(Ложь), ДекомпиляторФормы(Ложь, 2));
		СравнениеФайлов.ПоказатьРазличия();
	Иначе
		ТекстФормы = Новый ТекстовыйДокумент;
		ТекстФормы.УстановитьТекст(ДекомпиляторФормы(Ложь));
		ТекстФормы.Показать();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПостроитьДерево(пФормаКопия, мТекФорма, Дерево) Экспорт
	...
	Если мТекФорма.Панель.Страницы.Количество() > 1 Тогда
		Для Каждого СтраницаПанели Из мТекФорма.Панель.Страницы Цикл
			Если СтраницаПанели.Имя = "" Тогда
				Продолжить;
			КонецЕсли;
			Стр1Уровня = СтрокаФормы.Строки.Добавить();
			Стр1Уровня.Элемент			= СтраницаПанели;
			Стр1Уровня.ИмяЭлемента 		= СтраницаПанели.Имя;
			Стр1Уровня.ПолноеИмя		= "*ПанельФормы*_" + СтраницаПанели.Имя;
			Стр1Уровня.ТипЭлемента		= "Страница панели";
			Стр1Уровня.КартинкаФормы 	= ИндексКартинкиЭлФормы("Страница панели");
			Стр1Уровня.ТаблицаСвойств	= ЗаполнитьТаблицуСвойств(Стр1Уровня, мТекФорма);
			Стр1Уровня.ПривязкиЭлемента	= Новый ТаблицаЗначений;
			Стр1Уровня.ДействияЭлемента	= Новый ТаблицаЗначений;
		КонецЦикла;
	Иначе
		СтрокаФормы.ПолноеИмя = "*ПанельФормы*_" + мТекФорма.Панель.Страницы[0].Имя;
	КонецЕсли;
	...
	Для Сч = 0 По ПослСтрока Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрока.Страница) Тогда
			...
			Иначе
				ТекСтрока.Страница = Неопределено;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если РасширенныйАнализ = Истина Тогда
		...
		Попытка
			СоответствиеЗаголовковСтраниц = Новый Соответствие;
				СтрокаXPathСтраниц = "/elem[1]/elem[1]/elem[1]/elem[2]/elem[1]/elem[1]/elem/elem/data[5]";
				РезультатXPathСтраниц = ДокументDOM.ВычислитьВыражениеXPath(СтрокаXPathСтраниц, ДокументDOM,
				РазыменовательПИ, ТипРезультатаDOMXPath.НеупорядоченныйИтераторУзлов);
				Пока 1 = 1 Цикл
					...
					Попытка
						Если Лев(УзелСтраниц.ТекстовоеСодержимое, 1) = """" Тогда
							...
							СтруктураПанелиСтраницы = Новый Структура("Страница, Панель", СтрЗаменить(ИмяСтраницы, """", ""), "*ПанельФормы*");
							МассивСтраниц.Добавить(СтруктураПанелиСтраницы);
						КонецЕсли;
					Исключение
					КонецПопытки;
				КонецЦикла;
				Для Каждого Страница Из ОбрабатываемаяФорма.Панель.Страницы Цикл
...
Процедура ЗагрузитьПодчиненные(Узел, УзелРазбора, ТекущийУзел, СоответствиеКоманд)
	...
	Для ч = 1 По КоличествоПодчиненныхЭлементов * 2 Цикл
		...
		//чередуются узлы объявления базового типа элемента (с именем узла "data") и узлы с именем узла "elem" c информацией о самом элементе
...
Функция ДекомпиляторФормы(Бутафория, НомерФайлаСравнения = 1)  //из публикации http://infostart.ru/public/99363/ от Istur
	...
	Если НомерФайлаСравнения <> 1 Тогда
		...
		ДекФорма = ДеревоЭлементовФормыСравнения.Строки[0].Элемент;
	Иначе
		МассивСтрок = ДеревоЭлементовФормы.Строки[0].Строки.НайтиСтроки(Новый Структура("Пометка", Истина), Истина);
		ДекФорма = ДеревоЭлементовФормы.Строки[0].Элемент;
	КонецЕсли;
	Режим = "ТолькоСозданиеЭлементов";
	Для Каждого СтраницаФормы Из ДекФорма.Панель.Страницы Цикл
		Если СтраницаФормы <> ДекФорма.Панель.Страницы[0] И СтраницаФормы.Имя <> "" Тогда
			мТекст.ДобавитьСтроку("ЭтаФорма.Панель.Страницы.Добавить(""" + СтраницаФормы.Имя + """, """ + СтраницаФормы.Заголовок + """);");
		ИначеЕсли СтраницаФормы.Имя <> "" Тогда //переопределим свойства первой страницы
			Если СтраницаФормы.Имя <> "Страница1" Тогда
				мТекст.ДобавитьСтроку("ЭтаФорма.Панель.Страницы[0].Имя = """ + СтраницаФормы.Имя + """;");
			КонецЕсли;
			Если СтраницаФормы.Заголовок <> "Страница1" Тогда
				мТекст.ДобавитьСтроку("ЭтаФорма.Панель.Страницы[0].Заголовок = """ + СтраницаФормы.Заголовок + """;");
			КонецЕсли;
		КонецЕсли;
		Если СтраницаФормы.Видимость = Ложь Тогда
			мТекст.ДобавитьСтроку("ЭтаФорма.Панель.Страницы." + СтраницаФормы.Имя + ".Видимость = Ложь;");
		КонецЕсли;
		Если СтраницаФормы.Доступность = Ложь Тогда
			мТекст.ДобавитьСтроку("ЭтаФорма.Панель.Страницы." + СтраницаФормы.Имя + ".Доступность = Ложь;");
		КонецЕсли;
		Если ДекФорма.Панель.ОтображениеЗакладок <> ОтображениеЗакладок.НеИспользовать Тогда
			мТекст.ДобавитьСтроку("ЭтаФорма.Панель.ОтображениеЗакладок = ОтображениеЗакладок." + Строка(ДекФорма.Панель.ОтображениеЗакладок) + ";");
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрДерева Из МассивСтрок Цикл
		ОбработкаПрерыванияПользователя();
		ДобавитьСвойстваЭлемента(СтрДерева.ТипЭлемента, СтрДерева.Элемент, СтрДерева, Режим, Бутафория);
	КонецЦикла;
...
Процедура ДобавитьСвойстваЭлемента(ЭлементФормыСтр, Элемент, ЭлементТаблицы, Режим, Бутафория = Истина)
	...
	Если ЭлементФормыСтр = "Панель" Тогда
		...
		Если Режим <> "МодификацияСуществующихЭлементов" Тогда
			...
			Если ЭлементТаблицы.Панель = Неопределено ИЛИ ЭлементТаблицы.Панель = "*ПанельФормы*" Тогда
				
				Если ЭлементТаблицы.Страница <> Неопределено И ЭлементТаблицы.Страница <> "" Тогда
					СтрокаВставки = "ЭтаФорма.Панель.ТекущаяСтраница = ЭтаФорма.Панель.Страницы." + ЭлементТаблицы.Страница + ";";
...
Функция СоставитьСписокОбработчиковСобытий()
	...
	Список.Добавить("ОкончаниеПеретаскивания");              //53
...
Процедура СформироватьСписокОбработчиковСобытийУправляемойФормы()
	...
	СписокОбработчиковУправляемойФормы.Добавить("6d4d6747-a823-4f61-ab31-a426572f2c6c", "НачалоПеретаскивания");
	...
	СписокОбработчиковУправляемойФормы.Добавить("cb286ab3-3a1c-40d2-a232-6e64f624ccec", "ОкончаниеПеретаскивания");
	...
	СписокОбработчиковУправляемойФормы.Добавить("390d5e4b-e732-4c88-8748-9e211a416984", "ПриЧтенииНаСервере");
